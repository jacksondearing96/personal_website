...
function highlightLineage(itemName) {
  ...
  unHighlightGraph();
  let matchingNodes = nodes.filter(function (node) {
    return node.name === itemName;
  });
  matchingNodes
    .map((node) => node.id)
    .forEach((id) => highlightIds(getAllLineageSiblingIds(id)));
  ...
  for (i = 0; i < Math.min(searchResult.length, maxNumberAutocomplete); i++) {
    ...
    for (j = 0; j < searchResult[i].target.length; j++) {
      if (val.toLowerCase().includes(searchResult[i].target[j].toLowerCase())) {
        optElem.innerHTML +=
          "<strong>" + searchResult[i].target[j] + "</strong>";
      } else {
    ...
    optElem.innerHTML +=
      "<input type='hidden' value='" + searchResult[i].target + "'>";

    /* Insert the value for the autocomplete text field: */
    optElem.addEventListener("click", function (e) {
      document.getElementById("search-input").value = this.getElementsByTagName(
        "input"
      )[0].value;
      closeAllLists();
