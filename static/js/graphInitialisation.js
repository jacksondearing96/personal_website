...
function initialiseScrollPosition() {
  $("#visualisation-container").scrollTop(
    canvasHeight / 2 + scrollIncrementHeightToInitInCenter
  );
  $("#visualisation-container").scrollLeft(
    canvasWidth / 2 + scrollIncrementWidthToInitInCenter
  );
}
...
function generateForceDirectedSimulation() {
  ...
  simulation
    ...
    .force(
      "collision",
      d3.forceCollide((d) => (calculateNodeWidth(d) * collisionWidthRatio) / 2)
    )
