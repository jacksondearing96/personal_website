...
function calculateNodeWidth(node) {
  ...
  return Math.max(
    maxColumnWidth,
    topLevelWidth - 2 * topLevelNodePaddingHorizontal
  );
}
...
function calculateNodeHeight(node) {
  if (isTopLevelNode(node) && showColumns) {
    return (
      columnHeight * (countColumnsInGroup(node.group) - 1) +
      topLevelNodeTitleHeight +
      topLevelNodePaddingVertical
    );
  }
...
function getGridColumnWidth(columnIndex) {
  return nodesInGridColumn(columnIndex).map(calculateNodeWidth).max();
}
...
function setGridStartingWidths() {
  for (
    let columnIndex = 1;
    columnIndex <= maxGridColumnIndex();
    ++columnIndex
  ) {
    let prevIndex = columnIndex - 1;
    gridStartingWidths[columnIndex] =
      gridStartingWidths[prevIndex] +
      getGridColumnWidth(prevIndex) +
      topLevelNodeWidthBuffer;
  }
}
...
function hasAtLeastOneLink(node) {
  if (!isTopLevelNode(node))
    error("Testing independence of non-top-level node");
...
function setGridStartingHeights() {
  ...
  for (let columnIndex = 0; columnIndex <= maxGridColumnIndex_; ++columnIndex) {
    ...
    for (
      let verticalOrder = 0;
      verticalOrder <= maxVerticalOrder;
      ++verticalOrder
    ) {
      let height = 0;
      if (verticalOrder !== 0) {
        let prevNode = nodesInGridColumn(columnIndex).filter(
          (node) => node.verticalOrder === verticalOrder - 1
        )[0];
        height +=
          gridStartingHeights[getGridKey(columnIndex, verticalOrder - 1)];
...
function totalLinksSquaredDistance() {
  return links
    .map((link) => Math.pow(linkDistance(link), 2))
    .reduce((a, b) => a + b, 0);
}
    ...
    .forEach((columnIndex) => {
      verticalOrdersForColumn(columnIndex).forEach((verticalIndex) => {
        ...
        do {
          ...
          incrementDownStreamVerticalPadding(
            columnIndex,
            verticalIndex,
            optimisePaddingIncrement
          );
          ticked();
...
function incrementDownStreamVerticalPadding(
  columnIndex,
  verticalOrderStart,
  increment
) {
  let maxVerticalOrder = maxVerticalOrderForGridColumn(columnIndex);
  for (
    let verticalOrder = verticalOrderStart;
    verticalOrder <= maxVerticalOrder;
    ++verticalOrder
  ) {
...
function getNodeX(node) {
  if (isTopLevelNode(node)) {
    return staticMode
      ? canvasWidth / 2 +
          gridStartingWidths[node.gridColumn] +
          (getGridColumnWidth(node.gridColumn) - calculateNodeWidth(node)) / 2
      : node.x;
  }
...
function getNodeY(node) {
  if (isTopLevelNode(node)) {
    ...
    return staticMode
      ? canvasHeight / 2 +
          gridStartingHeights[gridKey] +
          optimisedPadding[gridKey]
      : node.y;
  }
...
function getLabelX(node) {
  if (isTopLevelNode(node))
    return (
      ...
      (calculateNodeWidth(node) - calculateTextWidthWithPadding(node.name)) / 2
    );
...
function getLabelY(node) {
  if (isTopLevelNode(node))
    return getNodeY(node) + columnHeight / 2 + labelOffsetToReachCenter + 5;
  return getNodeY(node) + columnHeight / 2 + labelOffsetToReachCenter;
}

function getLinkSource(link) {
  return isTopLevelNode(link.source)
    ? link.source
    : showColumns
    ? link.source
    : getParentTable(link.source);
}

function getLinkTarget(link) {
  return isTopLevelNode(link.target)
    ? link.target
    : showColumns
    ? link.target
    : getParentTable(link.target);
}
...
function ticked() {
  nodeSelection.attr("x", getNodeX).attr("y", getNodeY);
  if (showColumnsChanged) nodeSelection.attr("height", calculateNodeHeight);

  labels.attr("x", getLabelX).attr("y", getLabelY);
